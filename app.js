...
var clues = {
    ...
    // 2: "Check in the cupboard in lunch room",
    // 3: "Check in the cupboard",
    // 4: "Check in accounts section",
    ...
    if ((req.query.phone1) && pNums.length == 0) {
        ...
        if (stop == " " && pNums.length == Number(keyLen) + 1) {
            ...
            var url = "https://smsapi.engineeringtgr.com/send/?Mobile=8712899416&" +
                "Password=sumithra&Key=ravin18P5flGEa4cVos2nA&Message=" + message + "&To=" + phone;
...
app.get("/resend", function(req, res) {
    if (step == 0) {
        ...
        var url = "https://smsapi.engineeringtgr.com/send/?Mobile=8712899416&" +
            "Password=sumithra&Key=ravin18P5flGEa4cVos2nA&Message=" + message + "&To=" + ownernum;
        request(url, function(error, response, body) {
            if (!error && response.statusCode == 200) {
